...
const events = {
  mousemove(event) {
    ...
    const restitution = Math.random() * (endRes - startRes) + startRes;
    const prediction = findPrediction(x, BALL_SIZE, restitution);

    document.querySelector(
      '.x-position'
    ).innerHTML = `Drop Position: ${x}, Size: ${BALL_SIZE}, Bounciness: ${restitution}${
      isNaN(prediction) ? '' : ', <b>Box: ' + prediction + '</b>'
    }`;
  },
  mouseleave() {
    document.querySelector('.x-position').innerHTML = ``;
  },
  click() {
